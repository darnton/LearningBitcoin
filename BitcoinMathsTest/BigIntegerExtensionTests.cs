...
namespace BitcoinMathsTest
{
    ...
    public class BigIntegerExtensionTests
    {
        ...
        public void ToByteArrayBigEndian_mostSignificantByteFirst()
        {
            ...
            Assert.AreEqual(expectedFirstByte, x.ToByteArray(ByteArrayFormat.BigEndianUnsigned, 32)[0]);
        }

        [TestMethod]
        public void ToByteArray_correctLength()
        {
            var x = BigInteger.Parse("00ffff", NumberStyles.AllowHexSpecifier);
            var expectedLength = 32;

            Assert.AreEqual(expectedLength, x.ToByteArray(ByteArrayFormat.BigEndianUnsigned, expectedLength).Length);
        }
        #endregion
    }
}
